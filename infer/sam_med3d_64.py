...
class MaskDecoder3D(nn.Module):
    ...
    #     self.adapter_loss = nn.BCELoss(reduction='none')
    #     self.adapter_mlp = nn.Sequential(nn.Linear(384, 12, bias=False), nn.ReLU(), nn.Linear(12, 48, bias=False))
    #     self.optimizer = optim.SGD(self.adapter_mlp.parameters(), lr=0.1)
    
    # def interactive_train(self, x0, x1, x2, points):
    #     click, label = points
    #     click_idx = (click / 2).round().long().clamp(0, 31)
    #     # print(click_idx)
    #     click_idx = click_idx[..., 0] * 32 * 32 + click_idx[..., 1] * 32 + click_idx[..., 2]
    #     n_clicks = click_idx.shape[1]
    #     if n_clicks == 1:
    #         return 0.1 * self.adapter_mlp(x1)
    #     for param in self.adapter_mlp[0].parameters():
    #         nn.init.normal_(param, std=0.1)
    #     for param in self.adapter_mlp[2].parameters():
    #         nn.init.zeros_(param)
    #     points_labels = label.float()
    #     with torch.enable_grad():
    #         for epoch in range(self.adapter_epochs):
    #             x0new = x0 + 0.1 * self.adapter_mlp(x1)
    #             masks = (x0new @ x2.view(x2.shape[0], x2.shape[1], -1))
    #             masks_click = torch.gather(masks.expand(-1, n_clicks, -1), 2, click_idx[..., None])[..., 0]
    #             # print(masks_click.sigmoid(), points_labels)
    #             loss = self.adapter_loss(masks_click.sigmoid(), points_labels)
    #             loss = (loss - 0.65).clamp(0).mean()
    #             # weight = torch.arange(n_clicks, 0, -1, device=loss.device)
    #             # weight[0] = 0
    #             # loss = (loss * weight).sum() / weight.sum()
    #             self.optimizer.zero_grad()
    #             loss.backward()
    #             self.optimizer.step()
    #             # print(f'Epoch [{epoch + 1}/{self.adapter_epochs}], Loss: {loss.item():.4f}')
    #     return 0.1 * self.adapter_mlp(x1)
    ...
    def predict_masks(
        ...
        #     hyper_in += self.interactive_train(
        #         hyper_in.detach(),
        #         mask_tokens_out[:, 0, :].detach(),
        #         upscaled_embedding.detach(),
        #         points).detach()
        b, c, x, y, z = upscaled_embedding.shape  # [1, 48, 32, 32, 32]
        # print(mask_tokens_out.shape, upscaled_embedding.shape, hyper_in.shape)
...
class PositionEmbeddingRandom3D(nn.Module):
    ...
    def _pe_encoding(self, coords: torch.Tensor) -> torch.Tensor:
        ...
        # return torch.cat([torch.sin(coords), torch.sin(coords + torch.pi / 3), torch.sin(coords + 2 * torch.pi / 3)], dim=-1)
