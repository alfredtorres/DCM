...
def get_next_click3D_gaussian(prev_seg, gt_semantic_seg, K=6):
    ...
    pred_masks = prev_seg > mask_threshold
    true_masks = gt_semantic_seg > 0
    error_mask = pred_masks != true_masks

    gaussian_filter = GaussianFilter(spatial_dims=3, sigma=1.0, truncated=5.0)
    error_mask = gaussian_filter(error_mask.float())

    for i in range(gt_semantic_seg.shape[0]):
        flattened = error_mask[i].flatten()
        threshold = torch.topk(flattened, 64)[0][-1].clamp(0.3)
        high_prob_region = error_mask[i] >= threshold

        points = torch.argwhere(high_prob_region[0])
        if len(points) == 0:
            sel_points = torch.as_tensor(high_prob_region[0].shape).to(points) // 2 - 1
            sel_points = sel_points[None].expand(K, -1)
        elif len(points) <= K:  # With replacement (Allows duplicates)
            indices = torch.randint(0, len(points), (K,))
            sel_points = points[indices]
        else:  # Without replacement
            indices = torch.randperm(len(points))[:K]
            sel_points = points[indices]
        
        sel_labels = true_masks[i, 0, sel_points[:, 0], sel_points[:, 1], sel_points[:, 2]]

        batch_points.append(sel_points)
        batch_labels.append(sel_labels)

    batch_points = torch.stack(batch_points, dim=0).detach()
    batch_labels = torch.stack(batch_labels, dim=0).detach()
    return batch_points, batch_labels
